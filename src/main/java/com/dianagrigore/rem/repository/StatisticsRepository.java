...
public interface StatisticsRepository extends JpaRepository<Listing, String> {
    ...
    @Query("select new com.dianagrigore.rem.model.statistics.ListingsByOfferCount(count(listingId), listingId) from AgentListing group by listingId order by count(listingId)")
